...
if __name__ == '__main__':
    ...
    try:
        ...
        plt.subplot(1, 2, 1)
        plt.plot(losses)
        plt.title('Enhanced Model Training Loss (RGB)')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.grid(True)
        plt.yscale('log')
        ...
        plt.tight_layout()
        plt.savefig('enhanced_training_curves_rgb.png', dpi=300, bbox_inches='tight')
        plt.close()
        print("Enhanced training curves saved to enhanced_training_curves_rgb.png")
    except ImportError:
        print("Matplotlib not available, skipping loss curve plotting")

    print("\n=== Enhanced Model Summary (RGB) ===")
    print(f"✅ 总参数数量: {total_params:,}")
