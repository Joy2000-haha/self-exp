...
try:
    ...
    class EnhancedResidualBlock(nn.Module):
        ...
        def __init__(self, channels, num_layers=2, use_se=False):
            super().__init__()
            # 强制关闭SE注意力
            self.use_se = False
    ...
    class DownBlock2d(nn.Module):
        ...
        def __init__(self, in_channels, out_channels, num_layers, downsample=False, use_se=False):
            super().__init__()
            ...
            self.residual_blocks = nn.ModuleList([
                EnhancedResidualBlock(out_channels, 2, use_se=False) for _ in range(num_layers)
            ])
...
class EnhancedEncoder(nn.Module):
    ...
    def __init__(self, in_channels: int, layers: List[int], base_channels: int = 64) -> None:
        ...
        # 更深的特征提取网络 (移除SE注意力)
        self.layer1 = DownBlock2d(base_channels, base_channels, layers[0], downsample=False, use_se=False)
        self.layer2 = DownBlock2d(base_channels, base_channels * 2, layers[1], downsample=True, use_se=False)  # H/4
        self.layer3 = DownBlock2d(base_channels * 2, base_channels * 4, layers[2], downsample=True, use_se=False)  # H/8
...
class EnhancedPredictiveRepModel(nn.Module):
    ...
    def train_on_batch(self, obs1_input_b_l_c_h_w: torch.Tensor,
        ...
        # print("predicted_obs2_resized: min =", predicted_obs2_resized.min().item(), ", max =",
        #       predicted_obs2_resized.max().item())
        # print("predicted_norm: min =", predicted_norm.min().item(), ", max =", predicted_norm.max().item())
        # print("obs2_target_for_loss_b_c_h_w: min =", obs2_target_for_loss_b_c_h_w.min().item(), ", max =",
        #       obs2_target_for_loss_b_c_h_w.max().item())
...
if __name__ == '__main__':
    ...
    # 创建保存模型的目录
    save_dir = '../enhanced_model_checkpoints_no_se'
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % save_interval == 0:
            checkpoint_path = os.path.join(save_dir, f'enhanced_model_no_se_epoch_{epoch + 1}.pth')
            torch.save({
                'epoch': epoch + 1,
            ...
            print(f'Saved enhanced model checkpoint (no SE) to {checkpoint_path}')

    # 保存最终模型
    final_model_path = os.path.join(save_dir, 'enhanced_model_no_se_final.pth')
    torch.save({
        'model_state_dict': model.state_dict(),
        'model_config': {
            'img_in_channels': IMG_C,
    ...
    print(f'Saved final enhanced model (no SE) to {final_model_path}')
    ...
    try:
        ...
        plt.subplot(1, 2, 1)
        plt.plot(losses)
        plt.title('Enhanced Model Training Loss (No SE)')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.grid(True)
        plt.yscale('log')
        ...
        plt.tight_layout()
        plt.savefig('enhanced_training_curves_no_se.png', dpi=300, bbox_inches='tight')
        plt.close()
        print("Enhanced training curves (no SE) saved to enhanced_training_curves_no_se.png")
    except ImportError:
        print("Matplotlib not available, skipping loss curve plotting")

    print("\n=== Enhanced Model Summary (No SE Attention) ===")
    print(f"✅ 总参数数量: {total_params:,}")
    ...
    print("Enhanced model training completed (without SE attention)!")
